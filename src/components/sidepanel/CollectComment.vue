<template>
  <div class="container">
    <van-cell-group inset>
      ...
      />
      <van-field v-model="num" type="number" label="导出数量" :rules="[{ required: true }]" />
    </van-cell-group>
...
const props = defineProps<{
  submit: (urls: string[], num: number) => void;
}>();
...
const start = () => {
  ...
  props.submit(users, num.value);
};

// 是否为合法的博主链接
function isValidXHSProfileUrl(url: string) {
  return /^https:\/\/www\.xiaohongshu\.com\/user\/profile\/[a-zA-Z0-9]+(\?[^ ]*)?$/.test(url);
}
</script>
